      ...
      <Card className="shadow-premium hover-lift border-border/50 bg-gradient-to-br from-card/95 to-card/80 backdrop-blur-sm overflow-hidden">
        ...
        <CardContent className="relative z-10">
          <div className="space-y-4">
            <div>
              <p className="text-sm font-semibold text-muted-foreground mb-3">Áreas de Competencia:</p>
              <div className="flex flex-wrap gap-2">
                {Object.entries(coloresPorArea).map(([key, config]) => {
                  ...
                  return (
                    <Badge
                      key={key}
                      className="cursor-pointer hover:scale-105 transition-all duration-200 shadow-sm font-medium text-white border-0"
                      style={{
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {
             eventosHoy.length > 0 ? eventosHoy.map((evento) => {
              ...
              return (
                <div
                  key={evento.id}
                  className={`p-4 rounded-lg border-2 transition-all duration-300 ${isOngoing
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {eventosSiguiente.map((evento) => {
                ...
                return (
                  <div
                    key={evento.id}
                    className="p-4 rounded-lg border-2 bg-blue-500/5 border-blue-500/30 transition-all duration-300 hover:shadow-lg"
                  >
                    <div className="flex items-start justify-between gap-3 mb-3">
                      <div className="flex items-center gap-2">
                        {AreaIcon && <AreaIcon className="h-5 w-5" style={{ color: areaConfig?.darkBg }} />}
