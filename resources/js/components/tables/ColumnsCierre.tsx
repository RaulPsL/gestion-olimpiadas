  ...
  setFechaFin: Dispatch<React.SetStateAction<Date>>,
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex justify-center">
            <Badge
              className={`capitalize font-semibold ${config.bg} ${config.text} border ${config.border} px-3 py-1 ${estado === "en curso" ? "animate-pulse" : ""
                }`}
            >
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const now = new Date();
        const handleCierreValido = (fila: any): boolean => {
          const now = new Date();
          const fechaInicio = new Date(fila.fecha_inicio_fase);
          const fechaFin = new Date(fila.fecha_fin_fase);
          const fechaCalificacion = new Date(fila.fecha_calificacion_fase);
          if (fechaCalificacion < now || now >= fechaCalificacion) {
            console.log(`Fase: ${fila.fase} No se puede cerar por fecha de calificacion`);
            return false;
          }
          if (now >= fechaFin) {
            console.log(`Fase: ${fila.fase} No se puede cerar por fecha fin`);
            return false;
          }
          if (fila.estado === "finalizada" || fila.estado === "en curso") {
            console.log(`Fase: ${fila.fase} No se puede cerar por estado`);
            return false;
          }
          console.log(`Fase: ${fila.fase} Si se puede cerrar`);
          return true;
        }

        const handleCierreEda = (fila: any, funCierre: (fila: any) => boolean): boolean => {
          if (!funCierre(fila)) {
            return false;
          }
          if (fila.usuario_evaluador_id === "") {
            console.log(`Fase: ${fila.fase} Se puede cerar existe evaluador`);
            return false;
          }
          if (fila.usuario_encargado_id !== "") {
            console.log(`Fase: ${fila.fase} No se puede cerrar ya cerro un eda`);
            return false;
          }
          return true;
        }

        const handleCierreEva = (fila: any, funCierre: (fila: any) => boolean): boolean => {
          if (!funCierre(fila)) {
            return false;
          }
          if (fila.usuario_evaluador_id !== "") {
            console.log(`Fase: ${fila.fase} No se puede cerrar ya cerro un eva`);
            return false;
          }
          return true;
        }

        const cerrarEva = handleCierreEva(fila, handleCierreValido);

        const cerrarEda = handleCierreEda(fila, handleCierreValido);
        ...
        return (
          <div className="flex justify-center">
            <Button
              ...
              disabled={data?.rol?.sigla === "EDA" ? !cerrarEda : !cerrarEva}
              onClick={() => {
                setValue("fase_id", fila.fase_id);
                setValue(
